...
int FireSystem::findCriticalSpark(int& outIterations) const {
    outIterations = static_cast<int>(findCriticalSparkWorkload(1));
    ...
    return (num_threads == 1)
        ? n - 1
        : std::min(n - 1, chunk_size - 1);
}

long long FireSystem::findCriticalSparkWorkload(int repetitions) const {
    if (repetitions <= 0 || particles_.empty()) return 0;

    const int n = static_cast<int>(particles_.size());

    #ifndef _OPENMP
    long long sequentialOps = 0;
    for (int search = 0; search < repetitions; ++search) {
        for (int i = 0; i < n; ++i) {
            ++sequentialOps;
            if (i == n - 1) break;
        }
    }
    return sequentialOps;
    #else
    std::atomic<long long> global_ops(0);
    std::atomic<bool> found(false);

    const int num_threads = omp_get_max_threads();
    const int local_chunk = (n + num_threads - 1) / num_threads;
    // The target ends the first partition. Once one worker finds it, all
    // remaining repetitions terminate after the cancellation check.
    const int synthetic_target = std::min(n - 1, local_chunk - 1);

    #pragma omp parallel
    {
        int tid = omp_get_thread_num();
        int nth = omp_get_num_threads();
        const int workerChunk = (n + nth - 1) / nth;
        const int start = std::min(tid * workerChunk, n);
        const int end = std::min(start + workerChunk, n);
